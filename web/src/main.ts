...
const currentParams: Preset = { name: 'Demo (Auto-Cycle)', p: 1, q: 1, rad: 2.5, tube: 0.28 };
...
const pVal = document.getElementById('p-val') as HTMLSpanElement;
const qVal = document.getElementById('q-val') as HTMLSpanElement;
...
function updateHopfionGeometry(p: number, q: number, radius: number, tube: number): void {
...
function syncUI(p: number, q: number): void {
...
function applyPresetValues(preset: Preset, name: string): void {
...
function toggleDemo(forceState: boolean | null = null): void {
...
function handleManualSliderChange(): void {
